...
function androidFiles(name,id){return {
  ...
  'app/src/main/AndroidManifest.xml': `<manifest xmlns:android="http://schemas.android.com/apk/res/android"><application android:theme="@style/AppTheme" android:label="${name}"><activity android:name=".MainActivity" android:exported="true" android:launchMode="singleTop"><intent-filter><action android:name="android.intent.action.MAIN"/><category android:name="android.intent.category.LAUNCHER"/></intent-filter></activity></application></manifest>\n`,
  ...
  ['app/src/main/java/'+id.replaceAll('.','/')+'/MainActivity.java']: `package ${id};\nimport android.app.Activity; import android.os.Bundle; import android.content.Intent; import android.widget.TextView;\npublic class MainActivity extends Activity { private TextView view; public void onCreate(Bundle state){ super.onCreate(state); view=new TextView(this); setContentView(view); apply(getIntent()); } protected void onNewIntent(Intent intent){ super.onNewIntent(intent); setIntent(intent); apply(intent); } private void apply(Intent intent){ String payload=intent.getStringExtra("payload"); String token=intent.getStringExtra("pairingToken"); view.setText(payload==null?"Velocity Android Ready":payload); view.setContentDescription(token==null?"velocity-proof":"velocity-proof:"+token); } }\n`
};}
